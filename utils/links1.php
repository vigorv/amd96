...
while  ($row = $db->get_row($infoSQL)) {
    ...
    if (isset($xdata['direct_links'])){
       ...
       // preg_match_all("/[a-zA-Z0-9\/]+/", $dlinks_data,$data);
        /*$ids =array();
        foreach ($data as &$matches)
            foreach($matches as &$str)
                $ids[] = (int)substr($str,14);
        ...
        */
        ...
        foreach ($data as &$matches){
              //  echo($matches[0]);
                $dlinks_data = str_replace($matches[0],$matches[0].'<a href="http://fastlink.ws/catalog/file/'.$matches[1].'/1"><img src="http://fastlink.ws/img/fastlink.jpg"></a>',$dlinks_data);
        }
        if (count($data))
        print_r($dlinks_data);
        ...
        /*
        foreach ($ids as $id){
            $links.='<a href="http://fastlink.ws/catalog/viewv/'.$id.'">'.$id.'</a><br/>';
        }
        echo $links;*/
        /*
       //var_dump ($data2);
