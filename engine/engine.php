...
//forum		
	case "lb_discuss":
    	include ENGINE_DIR . '/modules/logicboard/discuss.php';
    	break;
///forum
	...
	default :
		...
		if ($do == "cat" and $category != '' and $subaction == '') {
			...
			if (isset ( $view_template ) and $view_template == "rss") {
				...
				$sql_select = "SELECT id, autor, date, short_story, full_story, xfields, title, title2, category, alt_name FROM " . PREFIX . "_post WHERE {$where_category} AND approve=1" . $where_date . " ORDER BY date DESC LIMIT 0," . $config['rss_number'];
			...
			if (isset ( $view_template ) and $view_template == "rss") {
				...
				$sql_select = "SELECT id, autor, date, short_story, full_story, xfields, title, title2, category, alt_name FROM " . PREFIX . "_post where {$stop_list}autor = '$user' AND approve=1" . $where_date . " ORDER BY date DESC LIMIT 0," . $config['rss_number'];
			...
			if ($year == '' and $month == '' and $day == '') {
				...
				if (isset ( $view_template ) and $view_template == "rss") {
					...
					$sql_select = "SELECT id, autor, date, short_story, full_story, xfields, title, title2, category, alt_name FROM " . PREFIX . "_post WHERE {$stop_list}approve=1";
...
elseif ($do == 'faq') $nam_e = "Часто задаваемые вопросы";
