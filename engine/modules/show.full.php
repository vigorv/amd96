...
if( ! $row['approve'] and $member_id['name'] != $row['autor'] and $member_id['user_group'] != '1' )
    $perm = 0;
if( ! $row['approve'] )
...
if( $row['id'] and $perm ) {
    ...
    if( $allow_full_cache and ! $full_cache )
        create_cache( $cprefix, serialize( $row ), $sql_news );

    $xfields = xfieldsload();

    if( $row['votes'] and $view_template != "print" )
        include_once ENGINE_DIR . '/modules/poll.php';
    ...
    elseif( $category_id and $cat_info[$category_id]['full_tpl'] != '' )
        $tpl->load_template( $cat_info[$category_id]['full_tpl'] . '.tpl' );
    else
        ...
        if( $config['seo_type'] == 1 or $config['seo_type'] == 2 ) {
            ...
            if( $category_id and $config['seo_type'] == 2 ) {
                ...
                if( $config['seo_control'] and ( isset( $_GET['seourl'] ) or strpos( $_SERVER['REQUEST_URI'], "?" ) !== false ) ) {
                    ...
                    if( $_GET['seourl'] != $row['alt_name'] or $_GET['seocat'] or $_GET['news_name'] or strpos( $_SERVER['REQUEST_URI'], "?" )
                        !== false ) {
    ...
    if( isset( $view_template ) and $view_template == "print" ) {
        ...
        if( $anzahl_seiten > 1 ) {
            ...
            if( $news_page < $anzahl_seiten ) {
                ...
                if( $config['allow_alt_url'] == "yes" ) {
                    $nextpage = " | <a href=\"" . $short_link . "page," . $pages . "," . $row['alt_name'] . ".html\">" . $lang['news_next'] .
                        "</a>";
                } else {
            ...
            if( $news_page > 1 ) {
                ...
                if( $config['allow_alt_url'] == "yes" ) {
                    $prevpage = "<a href=\"" . $short_link . "page," . $pages . "," . $row['alt_name'] . ".html\">" . $lang['news_prev'] .
                        "</a> | ";
                } else {
...
if ($logicboard_conf['discuss_status'])
{
    ...
    $tpl->set( '{LB_discuss_link}', $config['http_home_url']."?do=lb_discuss&id=".$row['id'] );
}
else
{
    $tpl->set_block( "'\\[LB_discuss\\](.*?)\\[/LB_discuss\\]'si", "" );
}
########### End. LogicBoard (DLE Edition) - ShapeShifter ###########
    ...
    if( $row['view_edit'] and $row['editdate'] and $zone) {
    ...
    if( $config['allow_alt_url'] == "yes" ) {
        ...
        $tpl->set( '[day-news]', "<a href=\"$PHP_SELF?year=" . date( 'Y', $row['date'] ) . "&amp;month=" . date( 'm', $row['date'] ) .
            "&amp;day=" . date( 'd', $row['date'] ) . "\" >" );
    ...
    $tpl->set( '{author}', "<a onclick=\"ShowProfile('" . urlencode( $row['autor'] ) . "', '" . $go_page . "', '" . $user_group[$member_id['user_group']]['admin_editusers'] .
        "'); return false;\" href=\"" . $go_page . "\">" . $row['autor'] . "</a>" );
    ...
    } elseif( $is_logged and ( ( $member_id['name'] == $row['autor'] and $user_group[$member_id['user_group']]['allow_edit'] ) or
...
$block_id = 'related_'.$row['id']; //РќРµ С‚СЂРѕРіР°С‚СЊ!
$relatedpro = "1"; //РќРµ С‚СЂРѕРіР°С‚СЊ!
$template = "relatednews"; // Р?РјСЏ С€Р°Р±Р»РѕРЅР° (РјРѕР¶РЅРѕ РјРµРЅСЏС‚СЊ)
$show_cat = "this";
$news_num = "9"; 
...
/*Block.Pro - relatednews*/
    ...
    if( $config['related_news'] and $view_template != "print" and strpos( $tpl->copy_template, "{related-news}" ) !== false ) {
        ...
        if( $allow_full_cache )
            $buffer = dle_cache( "related", $row['id'] . $config['skin'], true );
        else
            $buffer = false;
        ...
        if( $buffer === false ) {
            ...
            if( $row['related_ids'] ) {
                $db->query( "SELECT id, date, short_story, xfields, title, title2, category, alt_name FROM " . PREFIX . "_post WHERE id IN({$row['related_ids']}) ORDER BY id DESC" );
                $first_show = false;
                ...
                foreach( $user_group as $value ) {
                    if( $value['allow_cats'] != "all" and ! $value['allow_short'] )
                        $allowed_cats[] = $db->safesql( $value['allow_cats'] );
                }
                ...
                $db->query( "SELECT id, date, short_story, xfields, title, title2, category, alt_name FROM " . PREFIX . "_post WHERE {$allowed_cats}MATCH (title, short_story, full_story, xfields) AGAINST ('$body') AND id != " .
            ...
            while( $related = $db->get_row() ) {
                ...
                if( dle_strlen( $related['title'], $config['charset'] ) > 75 )
                ...
                if( $config['allow_alt_url'] == "yes" ) {
                    ...
                    if( $config['seo_type'] == 1 or $config['seo_type'] == 2 ) {
                        ...
                        if( $related['category'] and $config['seo_type'] == 2 ) {
                            ...
                            $full_link = $config['http_home_url'] . get_url( $related['category'] ) . "/" . $related['id'] . "-" . $related['alt_name'] .
                                ".html";
                ...
                if( $user_group[$member_id['user_group']]['allow_hide'] )
                    ...
                    $related['short_story'] = preg_replace( "#\[hide\](.+?)\[/hide\]#ims", "<div class=\"quote\">" . $lang['news_regus'] .
                        "</div>", $related['short_story'] );
                ...
                if( stripos( $tpl2->copy_template, "{image-" ) !== false ) {
                    ...
                    foreach( $data as $url ) {
                        ...
                        if( isset( $info['extension'] ) ) {
                            ...
                            if( ( $info['extension'] == 'jpg' ) || ( $info['extension'] == 'jpeg' ) || ( $info['extension'] == 'gif' ) || ( $info['extension'] ==
                                'png' ) )
                ...
                if( preg_match( "#\\{text limit=['\"](.+?)['\"]\\}#i", $tpl2->copy_template, $matches ) ) {
                    ...
                    if( $count and dle_strlen( $related['short_story'], $config['charset'] ) > $count ) {
                        ...
                        if( ( $temp_dmax = dle_strrpos( $related['short_story'], ' ', $config['charset'] ) ) )
                            $related['short_story'] = dle_substr( $related['short_story'], 0, $temp_dmax, $config['charset'] );
                    ...
                    $tpl2->set( '{text}', $related['short_story'] );
                ...
                if( $xfound ) {
                    ...
                    foreach( $xfields as $value ) {
                        ...
                        if( empty( $xfieldsdata[$value[0]] ) ) {
                            $tpl2->copy_template = preg_replace( "'\\[xfgiven_{$preg_safe_name}\\](.*?)\\[/xfgiven_{$preg_safe_name}\\]'is", "", $tpl2->
                                copy_template );
                            $tpl2->copy_template = str_replace( "[xfnotgiven_{$preg_safe_name}]", "", $tpl2->copy_template );
                            $tpl2->copy_template = str_replace( "[/xfnotgiven_{$preg_safe_name}]", "", $tpl2->copy_template );
                        } else {
                            $tpl2->copy_template = preg_replace( "'\\[xfnotgiven_{$preg_safe_name}\\](.*?)\\[/xfnotgiven_{$preg_safe_name}\\]'is", "", $tpl2->
                                copy_template );
                            $tpl2->copy_template = str_replace( "[xfgiven_{$preg_safe_name}]", "", $tpl2->copy_template );
                            $tpl2->copy_template = str_replace( "[/xfgiven_{$preg_safe_name}]", "", $tpl2->copy_template );
                        }

                        $tpl2->copy_template = str_replace( "[xfvalue_{$preg_safe_name}]", stripslashes( $xfieldsdata[$value[0]] ), $tpl2->
                            copy_template );
                    }
                }
            ...
            if( $allow_full_cache )
                create_cache( "related", $buffer, $row['id'] . $config['skin'], true );
        }
    ...
    if( $is_logged ) {
        ...
        if( ! in_array( $row['id'], $fav_arr ) )
            $tpl->set( '{favorites}', "<a id=\"fav-id-" . $row['id'] . "\" href=\"$PHP_SELF?do=favorites&amp;doaction=add&amp;id=" . $row['id'] .
                "\"><img src=\"" . $config['http_home_url'] . "templates/{$config['skin']}/dleimages/plus_fav.gif\" onclick=\"doFavorites('" .
                $row['id'] . "', 'plus'); return false;\" title=\"" . $lang['news_addfav'] . "\" style=\"vertical-align: middle;border: none;\" alt=\"\" /></a>" );
        else
            $tpl->set( '{favorites}', "<a id=\"fav-id-" . $row['id'] . "\" href=\"$PHP_SELF?do=favorites&amp;doaction=del&amp;id=" . $row['id'] .
                "\"><img src=\"" . $config['http_home_url'] . "templates/{$config['skin']}/dleimages/minus_fav.gif\" onclick=\"doFavorites('" .
                $row['id'] . "', 'minus'); return false;\" title=\"" . $lang['news_minfav'] . "\" style=\"vertical-align: middle;border: none;\" alt=\"\" /></a>" );
    ...
    if( strpos( $tpl->copy_template, "[category=" ) !== false ) {
        $tpl->copy_template = preg_replace( "#\\[category=(.+?)\\](.*?)\\[/category\\]#ies", "check_category('\\1', '\\2', '{$row['category']}')",
            $tpl->copy_template );
    }

    if( strpos( $tpl->copy_template, "[not-category=" ) !== false ) {
        $tpl->copy_template = preg_replace( "#\\[not-category=(.+?)\\](.*?)\\[/not-category\\]#ies", "check_category('\\1', '\\2', '{$row['category']}', false)",
            $tpl->copy_template );
    }
    ...
    $tpl->set( '{title2}', $metatags['title2'] );

    $row['short_story'] = stripslashes( $row['short_story'] );
    if( $user_group[$member_id['user_group']]['allow_admin'] != 1 )
    ...
    if( preg_match( "#\\{full-story limit=['\"](.+?)['\"]\\}#i", $tpl->copy_template, $matches ) ) {
        ...
        $tpl->set( '{full-story}', "<div id=\"news-id-" . $row['id'] . "\" style=\"display:inline;\">" . $row['full_story'] .
            "</div>" );
	#################################
	## Обработка линков и чето еще ##
	#################################
	## BEGIN ########################
	#################################
        if ($zone) {
            ...
            preg_match_all("/catalog\/viewv\/(?P<id>[0-9]+)[\"\\ =a-z_A-Z0-9\.а-яА-я\>]+\<\/a\>/", $dlinks_data,$data,PREG_SET_ORDER);
            ...
            $srcStr   = str_replace(PHP_EOL,'<br />',$dlinks_data );
                ...
                if (isset($xdata['m_direct_links'])){
                    $srcStr = makeFlow($xdata['m_direct_links']);
                }
            }
        } else {
            $srcStr = $row['src_links'];
        }
    ...
    if( strpos( $tpl->copy_template, "[xfvalue_" ) !== false or strpos( $tpl->copy_template, "[xfgiven_" ) !== false ) {
        ...
        foreach( $xfields as $value ) {
            ...
            if( empty( $xfieldsdata[$value[0]] ) ) {
                $tpl->copy_template = preg_replace( "'\\[xfgiven_{$preg_safe_name}\\](.*?)\\[/xfgiven_{$preg_safe_name}\\]'is", "", $tpl->
                    copy_template );
                $tpl->copy_template = str_replace( "[xfnotgiven_{$preg_safe_name}]", "", $tpl->copy_template );
                $tpl->copy_template = str_replace( "[/xfnotgiven_{$preg_safe_name}]", "", $tpl->copy_template );
            } else {
                $tpl->copy_template = preg_replace( "'\\[xfnotgiven_{$preg_safe_name}\\](.*?)\\[/xfnotgiven_{$preg_safe_name}\\]'is", "", $tpl->
                    copy_template );
                $tpl->copy_template = str_replace( "[xfgiven_{$preg_safe_name}]", "", $tpl->copy_template );
                $tpl->copy_template = str_replace( "[/xfgiven_{$preg_safe_name}]", "", $tpl->copy_template );
            }

            $tpl->copy_template = str_replace( "[xfvalue_{$preg_safe_name}]", stripslashes( $xfieldsdata[$value[0]] ), $tpl->
                copy_template );
        }
    }
    ...
    if( $user_group[$member_id['user_group']]['allow_hide'] )
        ...
        $tpl->result['content'] = preg_replace( "#\[hide\](.+?)\[/hide\]#ims", "<div class=\"quote\">" . $lang['news_regus'] .
...
if( $allow_comments and $news_found ) {
    ...
    if( ($zone) && ($comments_num > 0) ) {
        ...
        $comments->query = "SELECT " . PREFIX . "_comments.id, post_id, " . PREFIX . "_comments.user_id, date, autor as gast_name, " .
            PREFIX . "_comments.email as gast_email, text, ip, is_register, name, " . USERPREFIX .
            "_users.email, news_num, comm_num, user_group, lastdate, reg_date, signature, foto, fullname, land, icq, xfields FROM " .
            PREFIX . "_comments LEFT JOIN " . USERPREFIX . "_users ON " . PREFIX . "_comments.user_id=" . USERPREFIX .
            "_users.user_id WHERE " . PREFIX . "_comments.post_id = '$news_id'" . $where_approve . " ORDER BY date " . $config['comm_msort'];
    ...
    if( $is_logged and $config['comments_restricted'] and ( ( $_TIME - $member_id['reg_date'] ) < ( $config['comments_restricted'] *
        86400 ) ) ) {
    ...
    if( $member_id['restricted'] and $member_id['restricted_days'] and $member_id['restricted_date'] < $_TIME ) {
        ...
        $db->query( "UPDATE LOW_PRIORITY " . USERPREFIX .
            "_users SET restricted='0', restricted_days='0', restricted_date='' WHERE user_id='{$member_id['user_id']}'" );

    }

    if( $user_group[$member_id['user_group']]['allow_addc'] and $config['allow_comments'] == "yes" and $allow_add and ( $member_id['restricted'] !=
        2 and $member_id['restricted'] != 3 ) ) {
        ...
        if( $user_group[$member_id['user_group']]['comments_question'] ) {
            ...
            $tpl->set( '{question}', "<span id=\"dle-question\">" . htmlspecialchars( stripslashes( $question['question'] ), ENT_QUOTES ) .
                "</span>" );
        ...
        $tpl->copy_template = "<form  method=\"post\" name=\"dle-comments-form\" id=\"dle-comments-form\" action=\"{$_SESSION['referrer']}\">" .
            $tpl->copy_template . "
...
HTML;
		...
		/* Zone */
		 ...
		 //else {$tpl->result['content'] .= $tpl->result['addcomments'];}
