...
if( $allow_active_news ) {
	...
	while ( $row = $db->get_row( $sql_result ) ) {
		...
		if( isset($view_template) AND $view_template == "rss" ) {
			...
			$tpl->set( '{title2}', htmlspecialchars( strip_tags( stripslashes( $row['title2'] ) ) ) );
			...
			if ($row['title2'] == "")
				{
					...
					{
						$tpl->set( '{title2}', "<h2><a href=\"" . $full_link . "\">".stripslashes( $row['title2']."</a></h2>" ) );
					}
