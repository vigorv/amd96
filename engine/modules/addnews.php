...
if( ! $allow_addnews ) {
	...
	if( isset( $_REQUEST['mod'] ) and $_REQUEST['mod'] == "addnews" and $is_logged and $user_group[$member_id['user_group']]['allow_adds'] ) {
		...
		if ( !$user_group[$member_id['user_group']]['allow_html'] ) {
            ...
            $_POST['src_links'] = strip_tags ($_POST['src_links']);
		...
		if( $config['allow_site_wysiwyg'] == "yes" ) {
            ...
            $src_links = $db->safesql( $parse->BB_Parse( $parse->process( $_POST['src_links'] ) ) );
			$allow_br = 0;
            ...
            $src_links = $db->safesql( $parse->BB_Parse( $parse->process( $_POST['src_links'] ), false ) );
			$allow_br = 1;
		...
		$title2 = $db->safesql( $parse->process( trim( strip_tags ($_POST['title2']) ) ) );
        $src_link = $db->safesql( $parse->process( trim( strip_tags ($_POST['src_link']) ) ) );
		...
		if( $title2 == "" or ! $title2 ) $stop .= $lang['add_err_1'];
		if( dle_strlen( $title2, $config['charset'] ) > 200 ) $stop .= "title2 more than 200 symbols";
		...
		if( ! $stop ) {
			...
			if( $found ) {
				...
				$db->query( "UPDATE " . PREFIX . "_post set title='$title', title2='$title2', short_story='$short_story', full_story='$full_story', xfields='$filecontents', category='$category_list', alt_name='$alt_name', allow_comm='$allow_comm', approve='$approve', allow_main='$allow_main', fixed='$news_fixed', allow_br='$allow_br', tags='" . $_POST['tags'] . "' WHERE id='$id'" );
                ...
                if (isset($_POST['xfield']['direct_links'])) {
                    //var_dump($_POST['xfield']['direct_links']);
                    $direct_links=$_POST['xfield']['direct_links'];
                    preg_match_all('/\/catalog\/file\/([0-9]+)/', $direct_links, $matches, PREG_SET_ORDER);
                    $data = array();
                    foreach ($matches as $it) {
                        $data['ids'][] = $it[1];
                    }
//        var_dump($direct_links);
//        var_dump($matches);
//        var_dump($data);
                    $data['gid'] = $row['id'];
                    $fdata = base64_encode(serialize($data));
                    $key = md5($config['service_key'] . $fdata . $config['service_uri']);
                    //var_dump($config['service_key']);
                    //var_dump($config['service_uri']);
		...
		if( $found ) {
			...
			$tpl->set( '{title2}', $parse->decodeBBCodes( $row['title2'], false ) );
			...
			$tpl->set('{title2}',$parse->decodeBBCodes( $row['title2'], false ) );
            $tpl->set('{src_link}',$parse->decodeBBCodes( $row['src_link'], false ) );
            $tpl->set('{src_links}',$parse->decodeBBCodes( $row['src_links'], TRUE, $config['allow_site_wysiwyg']));
        ...
        $tpl->set('{service_uri}',$config['service_uri']);
