...
if( ! $user_group[$member_id['user_group']]['allow_search'] ) {
  ...
  //  $config['search_length_min'] = 2;
    ...
    if( empty( $story ) AND !empty( $searchuser ) AND $titleonly != 2) $story = "___SEARCH___ALL___"; // Äëÿ ïîèñêà âñåõ ñòàòåé
    if( $search_start < 0 ) $search_start = 0; // Íà÷àëüíàÿ ñòðàíèöà ïîèñêà
    if( $titleonly < 0 or $titleonly > 3 ) $titleonly = 0; // Èñêàòü â çàãîëîâêàõ, ñòàòüÿõ, êîììåíòàðèÿõ
    ...
    if ($config['full_search']) {
        if( ! (in_array( $sortby, $listsortby )) ) $sortby = ""; // Ñîðòèðîâàòü ïî ïîëÿì
    } else {
        if( ! (in_array( $sortby, $listsortby )) ) $sortby = "date"; // Ñîðòèðîâàòü ïî ïîëÿì
    }
    ...
    if( $subaction == "search" ) {
        ...
        if( (empty( $story ) or ($story_maxlen < $config['search_length_min'])) and (empty( $searchuser ) or (strlen( $searchuser ) < $config['search_length_min'])) ) {
            ...
            if ($config['full_search']) {
                ...
                $titleonly_where = array ('0' => "MATCH(title,short_story,full_story,xfields) AGAINST ('{story}')", // Èñêàòü òîëüêî â ñòàòüÿõ
                ...
                $titleonly_where = array ('0' => "short_story LIKE '%{story}%' OR full_story LIKE '%{story}%' OR xfields LIKE '%{story}%' OR title LIKE '%{story}%'", // Èñêàòü òîëüêî â ñòàòüÿõ
