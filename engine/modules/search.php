...
// Ìèíèìàëüíîå êîëè÷åñòâî ñèìâîëîâ â ñëîâå ïîèñêà
    ...
    if (isset($_REQUEST['result_from']))
        ...
        $result_from = 1; // Ïîêàçàòü ñòðàíèöó ñ ðåçóëüòàòîì ¹ ÕÕÕ
    ...
    if (empty($story) AND !empty($searchuser) AND $titleonly != 2)
        $story = "___SEARCH___ALL___"; // Äëÿ ïîèñêà âñåõ ñòàòåé
    if ($search_start < 0)
        $search_start = 0; // Íà÷àëüíàÿ ñòðàíèöà ïîèñêà
    if ($titleonly < 0 or $titleonly > 3)
        $titleonly = 0; // Èñêàòü â çàãîëîâêàõ, ñòàòüÿõ, êîììåíòàðèÿõ
    if ($replyless < 0 or $replyless > 1)
        $replyless = 0; // Èñêàòü áîëüøå èëè ìåíüøå îòâåòîâ
    if ($replylimit < 0)
        $replylimit = 0; // Ëèìèò îòâåòîâ
    if ($showposts < 0 or $showposts > 1)
        $showposts = 0; // Èñêàòü â ñòàòüÿõ èëè êîììåíòàðèÿõ þçåðà
    ...
    if ($beforeafter != "after" and $beforeafter != "before")
        $beforeafter = "after"; // Èñêàòü äî èëè ïîñëå ïåðèîäà äíåé
    $listsortby = array("date", "title", "comm_num", "news_read", "autor", "category", "rating");

    if ($config['full_search']) {
        if (!(in_array($sortby, $listsortby)))
            $sortby = ""; // Ñîðòèðîâàòü ïî ïîëÿì
    } else {
        if (!(in_array($sortby, $listsortby)))
            $sortby = "date"; // Ñîðòèðîâàòü ïî ïîëÿì
    }

    $listresorder = array("desc", "asc");
    if (!(in_array($resorder, $listresorder)))
        $resorder = "desc"; // Ñîðòèðîâàòü ïî âîçðàñòàþùåé èëè óáûâàþùåé
...
// Ïîèñê ïî êëþ÷åâûì ñëîâàì
            ...
            if ($config['full_search']) {
                ...
                $titleonly_where = array('0' => "MATCH(title,short_story,full_story," . PREFIX . "_post.xfields) AGAINST ('{story}')", // Èñêàòü òîëüêî â ñòàòüÿõ
                ...
                $titleonly_where = array('0' => "short_story LIKE '%{story}%' OR full_story LIKE '%{story}%' OR " . PREFIX . "_post.xfields LIKE '%{story}%' OR title LIKE '%{story}%'", // Èñêàòü òîëüêî â ñòàòüÿõ
...
// ------ Çàïðîñ ê áàçå
            ...
            if ($sphinx_search) {
                ...
                // Ïîäñîåäèíÿåìñÿ ê Sphinx-ñåðâåðó
                $sphinx->SetServer('localhost', 3312);

                // Ñîâïàäåíèå ïî ëþáîìó ñëîâó
                $sphinx->SetMatchMode(SPH_MATCH_ALL);

                // Ðåçóëüòàòû ñîðòèðîâàòü ïî ðåëåâàíòíîñòè
                $sphinx->SetSortMode(SPH_SORT_RELEVANCE);

                // Çàäàåì ïîëÿì âåñà (äëÿ ïîäñ÷åòà ðåëåâàíòíîñòè)
                $sphinx->SetFieldWeights(array('title' => 20, 'title2' => 15, 'short_story' => 10, 'full_story' => 10));

                // Ðåçóëüòàò ïî çàïðîñó (* - èñïîëüçîâàíèå âñåõ èíäåêñîâ)
                $result = $sphinx->Query($story, 'rumedia_post');

                $limit = $config['search_number'];
                // Åñëè åñòü ðåçóëüòàòû ïîèñêà, òî
                if ($result && isset($result['matches'])) {
                    ...
                    $sql = "SELECT SQL_CALC_FOUND_ROWS id, autor, " . PREFIX . "_post.date AS newsdate, " . PREFIX . "_post.date AS date, short_story AS story, " . PREFIX . "_post.xfields AS xfields, title, descr, keywords, category, alt_name, comm_num AS comm_in_news, allow_comm, rating, news_read, editdate, editor, reason, view_edit, tags, '' AS output_comms FROM rm_post WHERE rm_post.approve=1
               AND `id` IN ($id_list) LEFT JOIN " . PREFIX . "_post_extras ON (" . PREFIX . "_post.id=" . PREFIX . "_post_extras.news_id) ORDER BY date  DESC Limit $search_start,$limit";
                    //echo $sql;
