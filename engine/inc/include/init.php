...
$allow_login = true;
if( $config['login_log'] )
    $allow_login = check_allow_login( $_IP, $config['login_log'] );
if( ! $allow_login )
...
if( $is_loged_in and $_REQUEST['subaction'] == 'dologin' ) {
    ...
    if( $config['login_log'] )
        $db->query( "DELETE FROM " . PREFIX . "_login_log WHERE ip = '{$_IP}'" );
    if( $config['log_hash'] ) {
        ...
        $member_id['hash'] = $hash;
        $db->query( "UPDATE " . USERPREFIX . "_users set hash='" . $hash . "', lastdate='{$time_now}', logged_ip='" . $_IP .
...
if( $is_loged_in and $config['ip_control'] == '1' and ! check_netz( $member_id['logged_ip'], $_IP ) and $_REQUEST['subaction'] !=
    'dologin' )
    $is_loged_in = false;
if( ! $is_loged_in ) {
    $member_id = array();
