...
if( $action == "list" ) {
	...
	while ( $row = $db->get_array() ) {
		...
		if( dle_strlen( $row['title2'], $config['charset'] ) > 65 ) $title = dle_substr( $row['title2'], 0, 65, $config['charset'] ) . " ...";
		else $title2 = $row['title2'];

		$title = htmlspecialchars( stripslashes( $title ), ENT_QUOTES );
		$title = str_replace("&amp;","&", $title );

		$title2 = htmlspecialchars( stripslashes( $title2 ), ENT_QUOTES );
...
elseif( $action == "editnews" ) {
	...
	$row['title2'] = $parse->decodeBBCodes( $row['title2'], false );
	$row['title2'] = str_replace("&amp;","&", $row['title2'] );
...
<table width="100%">
    <tr>
        ...
        <td>ID=<b>{$row['id']}</b>, {$lang['edit_eau']} {$author_info}&nbsp;&nbsp;&nbsp;<a href="http://rumedia.ws/download/trend/3.php?sgroup=2&amp;id={$row['id']}" class="adm-btn" onMouseover="showhint('Посмотреть статистику по кликам', this, event, '220px')">[?]</a></td>
    </tr>
    <tr>
    ...
    <tr>
        <td width="140" height="29" style="padding-left:5px;">{$lang['edit_et']}</td>
        <td><input class="edit bk" type="text" style="width:350px;" name="title2" id="title2" value="{$row['title2']}"></td>
    </tr>
    <tr>
...
<div class="hr_line"></div>
            ...
            <td height="29" width="140" style="padding-left:5px;">Data</td>
                    <td><a href="{$config['service_uri']}/site/upload" target="_blank" onClick="addData(); return false;">Добавить ссылки </a>
                    <a href="{$config['service_uri']}/catalog/filegroup/{$row['id']}" target="_blank" onClick="viewData();return false">Просмотреть связанные ссылки</a>
                    </td>
                    </tr>
                    </table>
            <script langauge="javascript">
            function addData(){
                 window.open('{$config['service_uri']}/site/upload?lay_mini=1', '_AddLinks', 'toolbar=0,location=0,status=0, left=0, top=0, menubar=0,scrollbars=yes,resizable=0,width=640,height=550');
            }
            function viewData() {
                window.open('{$config['service_uri']}/catalog/filegroup/{$row['id']}?lay_mini=1', '_ViewLinks', 'toolbar=0,location=0,status=0, left=0, top=0, menubar=0,scrollbars=yes,resizable=0,width=640,height=550');
            }
            </script>
<div class="hr_line"></div>
<table width="100%">
...
HTML;
    ...
    <td>{$bb_panel}<textarea rows="19" onclick="setFieldName(this.name)" name="full_story" id="full_story" style="width:98%;" class="bk">{$row['full_story']}</textarea>
		...
		<tr>
        <td>Источники</td>
        <td><input name="src_link" onclick="setFieldName(this.name)" id="src_link" style="width:98%;" class="bk" value="{$row['src_link']}"/></td>
    </tr>
	<tr>
        <td>Ссылки Источника</td>
        <td><textarea rows="19" name="alt_links" onclick="setFieldName(this.name)" id="alt_links" style="width:98%;" class="bk">{$row['src_links']}</textarea></td>
    </tr>
...
elseif( $action == "doeditnews" ) {
	...
	$title2 = $parse->process( trim( strip_tags ($_POST['title2']) ) );

	if ( !$user_group[$member_id['user_group']]['allow_html'] ) {

		$_POST['short_story'] = strip_tags ($_POST['short_story']);
		$_POST['full_story'] = strip_tags ($_POST['full_story']);
        $_POST['alt_links']= strip_tags ($_POST['alt_links']);
        $_POST['src_link']= strip_tags ($_POST['src_link']);
    ...
    $alt_links = $parse->process($_POST['alt_links']);
    $src_link = $parse->process($_POST['src_link']);
    ...
    $alt_links = $db->safesql($alt_links);
    $src_link = $db->safesql($src_link);
	...
	if( dle_strlen( $title2, $config['charset'] ) > 255 ) {
		msg( "error", $lang['cat_error'], $lang['addnews_ermax'], "javascript:history.go(-1)" );
	}
	...
	$db->query( "SELECT id, title, title2, autor, category, approve, tags, news_id FROM " . PREFIX . "_post LEFT JOIN " . PREFIX . "_post_extras ON (" . PREFIX . "_post.id=" . PREFIX . "_post_extras.news_id) WHERE id = '$id'" );
		...
		if( $have_perm ) {
			...
			if( $ifdelete != "yes" ) {
				...
				if( $_POST['allow_date'] != "yes" ) {
					...
					$db->query( "UPDATE " . PREFIX . "_post SET title='$title', title2='$title2', date='$thistime', short_story='$short_story', full_story='$full_story', xfields='$filecontents', descr='{$metatags['description']}', keywords='{$metatags['keywords']}', category='$category_list', alt_name='$alt_name', allow_comm='$allow_comm', approve='$approve', allow_main='$allow_main', fixed='$news_fixed', allow_br='$allow_br', symbol='$catalog_url', tags='{$_POST['tags']}', metatitle='{$metatags['title']}', src_link='{$src_link}', src_links='{$alt_links}' WHERE id='$item_db[0]'" );

				} else {

					$db->query( "UPDATE " . PREFIX . "_post SET title='$title', title2='$title2', short_story='$short_story', full_story='$full_story', xfields='$filecontents', descr='{$metatags['description']}', keywords='{$metatags['keywords']}', category='$category_list', alt_name='$alt_name', allow_comm='$allow_comm', approve='$approve', allow_main='$allow_main', fixed='$news_fixed', allow_br='$allow_br', symbol='$catalog_url', tags='{$_POST['tags']}', metatitle='{$metatags['title']}', src_link='{$src_link}', src_links='{$alt_links}' WHERE id='$item_db[0]'" );
				}
