...
//$id=12364;
if($id>0) $where=" l.catalog_group_id=".$id."  ";
if($sgroup>0) $where.=" and l.catalog_sgroup_id=".$sgroup."  ";
    ...
    $sql="
    SELECT 
	    l.catalog_group_id,min(l.created) as mindate,max(l.created) as maxdate,
...
//print_r($dates);
    ...
    $sql="SELECT  
	 ...
	 where catalog_group_id={$row['catalog_group_id']} and catalog_sgroup_id={$sgroup}
...
$sql="SELECT date_format(created,'%d.%m') as created FROM `post_changefiles` WHERE `post_id` ={$row['catalog_group_id']} and created>='{$min}' and created<='{$maxdd}' group by date_format(created,'%Y-%m-%d')";
//echo $sql;
...
if(count($serie4)>1)
prgrath($row['catalog_group_id'],$serie1,$serie5,$serie4,iconv('windows-1251','utf-8',$row['title']));
else echo "no graph\n<br>";
print_r($serie1);
}
