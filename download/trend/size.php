...
$sgroup=(int)$_REQUEST['sgroup'];if($sgroup>0) $where.=" and l.catalog_sgroup_id=".$sgroup."  ";
    $db1=mysql_connect("kunashir.hz.anka.ws", "fldbusr","akl,gfhjkm") or    die("Could not connect: " . mysql_error());
    mysql_select_db("fastlink2",$db1);
    mysql_query("SET NAMES ".COLLATE2,$db1);
    $db2=mysql_connect(DBHOST2, DBUSER2,DBPASS2) or    die("Could not connect: " . mysql_error());
    mysql_select_db(DBNAME2,$db2);
    mysql_query("SET NAMES ".COLLATE2,$db2);

    $sql ="select date_format(dt,'%Y-%m-%d') as created,sum(sz)/1024/1024/1024 as size from fl_catalog where dt >'2012-08-01' group by date_format(dt,'%Y-%m-%d')";
    $result=mysql_query($sql,$db1) or die("#1".mysql_error());
    echo "<pre>Upload to fastlink per date<br>";
while($row=mysql_fetch_assoc($result))
{
    echo $row['created']." =>".floor($row['size'])."Gb<br>";
}

    $sql ="select date_format(created,'%Y-%m-%d') as created,count(*) as size from fl_catalog_file_not_exists where created >'2012-08-01' group by date_format(created,'%Y-%m-%d')";
    $result=mysql_query($sql,$db2) or die("#1".mysql_error());
    echo "<pre>Not exist links<br>";
while($row=mysql_fetch_assoc($result))
{
    echo $row['created']." =>".floor($row['size'])."<br>";
}

    $sql ="select date_format(created,'%Y-%m-%d') as created,count(*) as size from fl_catalog_file_sv where created >'2012-08-01'
AND zone <>  '9' AND zone <>  '8,9' AND zone <>  '7,9' AND zone <>  '3,9'
group by date_format(created,'%Y-%m-%d')";
    $result=mysql_query($sql,$db2) or die("#1".mysql_error());
    echo "<pre>Not have a server<br>";
while($row=mysql_fetch_assoc($result))
{
    echo $row['created']." =>".floor($row['size'])."<br>";
}
