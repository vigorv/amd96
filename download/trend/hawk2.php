...
$id=(int)$_REQUEST['id'];
    $href = str_replace('http://', '', strtolower(substr($_REQUEST['href'], 0, 255)));
    $href = explode('/', $href);
    unset($href[0]);
    $href = addslashes(implode('/', $href));
        	    	    	        
//$id=12364;
if($id>0) $where=" p.id=" . $id . ' AND href="' . $href . '"';

    /* соединямеся с базой данных */
    mysql_connect("localhost", mysql_user, mysql_pass) or    die("Could not connect: " . mysql_error());
...
//print_r($dates);
    ...
    $sql="SELECT  
	 ...
	 FROM dle_loads  
	 where news_id={$row['id']} AND href='{$href}'
...
$sql="SELECT date_format(date,'%d.%m') as created FROM `dle_post_logs` WHERE `post_id` ={$row['id']} and date>='{$min}' and date<='{$maxdd}' group by date_format(date,'%Y-%m-%d')";
//echo $sql;
$result2=mysql_query($sql) or die(mysql_error());
$serie2=array();
