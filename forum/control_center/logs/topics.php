...
if (isset($_POST['del_checked']))
{
	...
	if ($_POST['act'] == 1)
	{
		...
		if ($selected AND control_center_admins($member_cca['logs']['topics_del']))
		{
			$j = 0;
			foreach	($selected as $id)
			{
				$j ++;
				$id = intval($id);
				$DB->delete("id = '{$id}'", "logs_topics");
			}

			$info = "<font color=red>Удаление</font> логов действий с темами (".$j.")";
			$DB->insert("member_name = '{$member_id['name']}', date = '{$time}', ip = '{$_IP}', module = '{$do}', op = '{$op}', info = '{$info}'", "logs_actions_cc");

			header( "Location: {$redirect_url}?do=logs&op=topics" );
            exit();
		}
        elseif(!control_center_admins($member_cca['logs']['topics_del']))
        {
...
if (isset($_REQUEST['where_search']) AND isset($_REQUEST['word']))
{
	...
	unset ($safehtml);
	$link_nav = $redirect_url."?do=logs&op=topics&where_search=".$ws."&word=".$word."&type=".$type."&page=";
}
else
{
	$word = "";
	$ws = "";
	$type = 0;
	$link_nav = $redirect_url."?do=logs&op=topics&page=";
}
...
if ($word AND $ws)
{
	...
	if ($ws == "mid" OR $ws == "act_st" OR $ws == "fid" OR $ws == "tid")
	{
        $where = "log.".$ws." = '$word'";
	}
	elseif ($ws == "date")
    ...
    $("#show_info").click(function () {
      ...
      $("div #info_info").show(500);      
                    ...
                    <div class="headerBlue">
                        ...
                        <div class="headerBlueBg">Действия с темами</div>
                    </div>
...
$DB->join_select( "log.*, u.user_id, u.name, t.title", "LEFT", "logs_topics log||users u||topics t", "log.mid=u.user_id||log.tid=t.id", $where, "ORDER BY log.date DESC LIMIT ".$page.", ".$log_result."" );
while ( $row = $DB->get_row() )
{
    ...
    $row['title'] = "<a href=\"".$redirect_url_board."?do=board&op=topic&id=".$row['tid']."\" target=\"blank\" title=\"Открыть страницу с темой. ID: ".$row['tid']."\">".sub_title($row['title'], 20)."</a>";
    ...
    if ($row['act_st'] == 0)
    {
        ...
        $logs_info = unserialize($row['info']);
        if (!$logs_info)
        {
            $row['title'] = $logs['info'];
        }
        else
        {
            ...
            $row['title']= $logs_info['title'];
        }
    }
    elseif ($row['act_st'] == 1) $act_st = "Отредактирвоана";
    elseif ($row['act_st'] == 2) $act_st = "Закрыта";
    elseif ($row['act_st'] == 3) $act_st = "Открыта";
    elseif ($row['act_st'] == 4) $act_st = "Закрпелена";
    elseif ($row['act_st'] == 5) $act_st = "Откреплена";
    elseif ($row['act_st'] == 6) $act_st = "Очищен лист подписчиков";
    elseif ($row['act_st'] == 7) $act_st = "Скрыта";
    elseif ($row['act_st'] == 8) $act_st = "Опубликована";
    elseif ($row['act_st'] == 9) $act_st = "Перемещена";
    elseif ($row['act_st'] == 10) $act_st = "Объединена";
    elseif ($row['act_st'] == 11) $act_st = "Добавлено голосование";
    elseif ($row['act_st'] == 12) $act_st = "Отредактирвоано голосование";
    elseif ($row['act_st'] == 13) $act_st = "Удалено голосование";
...
echo <<<HTML
                ...
                <td class="appBtn"><a href="#" onclick="window.open('{$redirect_url}?do=infopage&op=logs&type=topics&id={$row['id']}','Данные темы','width=500,height=430,toolbar=1,location=0,scrollbars=1'); return false;" title="Просмотреть подробную информацию."><img src="{$redirect_url}template/images/info_link.gif" alt="Подробнее..." /></a></td>
...
if ($i > 0)
{
	$nav = $DB->one_select( "COUNT(*) as count", "logs_topics log", $where);
	$nav_all = $nav['count'];
	$DB->free($nav);
